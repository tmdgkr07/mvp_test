...
export async function GET() {
  ...
  const items = [
    ...feedbackRows.map((item) => ({
      ...
      href: `/dashboard?hub=platform&projectId=${item.projectId}&tab=feedback`
    })),
    ...supportRows.map((item) => ({
      ...
      href: item.projectId ? `/dashboard?hub=platform&projectId=${item.projectId}&tab=rice` : "/dashboard?hub=platform&tab=rice"
    }))
  ]
